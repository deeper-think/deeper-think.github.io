...
## 1. 内存技术的发展
早期的计算机，程序直接运行在物理内存上，存在的问题是：

> 内存使用率低，运行的程序需要全部调入物理内存才可以运用，这有可能导致物理内存不足，32k内存的计算机没法运行64k的程序，另外一个是对于多任务系统,不同任务之间的隔离性也存在问题。                                                            
...
虚拟地址空间常被称为线性地址空间，虚拟地址也被称为现行地址。paging技术是实现虚拟内存的一种方式，paging技术的实现主要依赖于硬件MMU及内核页表技术，下面是从硬件及内核层面来观察不同模块所实现的功能。
